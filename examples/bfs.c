...
int bfs(Graph *graph, BfsVertex *start, List *hops)
{
   ...
   /* Initialie the queue with the adjacency list of the start vertex. */
   ...
   while(queue_size(&queue) > 0)
   {
      ...
      /* Traverse each vertex in the current adjacent list. */
      for(member = list_head(&adjlist->adjacent); member != NULL; member = list_next(member))
      {
         ...
         /* Color each white vertex gray and enqueue its adjacent list. */
   ...
   for(element = list_head(&graph_adjlists(graph)); element != NULL; element = list_next(element))
   {
      ...
      {
         if(list_ins_next(hops, list_tail(hops)) != 0)
         {
            list_destroy(hops);
            return -1;
         }
